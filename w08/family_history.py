...
def print_death_age(people_dict):
    ...
    for person in people_dict:
        ...
        death_age = people_dict[person][3] - people_dict[person][2]
        print(people_dict[person][0], death_age)
...
def count_genders(people_dict):
    ...
    for person in people_dict:
        ...
        if people_dict[person][1] == "M":
            amount_males += 1
        # Do we need to specify female in case of other geners?
        else:
            amount_females += 1
    
    print(f"Number of Males: {amount_males}")
    print(f"Number of Females: {amount_females}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage in marriages_dict:
        wife_key = marriages_dict[marriage][1]
        husband_key = marriages_dict[marriage][0]

        wife_info = people_dict[wife_key]
        husband_info = people_dict[husband_key]

        wife_age_wedding = marriages_dict[marriage][2] - wife_info[2]
        husband_age_wedding = marriages_dict[marriage][2] - husband_info[2]

        print(f"Husband: {husband_info[0]}\nAge at wedding: {husband_age_wedding}")
        print(f"Wedding Year: {marriages_dict[marriage][2]}")
        print(f"Wife: {wife_info[0]}\nAge at wedding: {wife_age_wedding}")
