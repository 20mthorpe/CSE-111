# Import datetime so that it can be
# used to compute a person's age.
from datetime import datetime


def main():
    # Get the user's gender, birthdate, height, and weight.
    gender = input("Are you Male or Female? ")
    birth_str = input("What is your birthdate?(YYYY-MM-DD) ")
    inches = float(input("What is your height in inches? "))
    pounds = float(input("What is your weight in pounds? "))
    # Call the compute_age, kg_from_lb, cm_from_in,
    # body_mass_index, and basal_metabolic_rate functions
    # as needed.
    years = compute_age(birth_str)
    kilograms = kg_from_lb(pounds)
    centimeters = cm_from_in(inches)
    bmi = body_mass_index(kilograms, centimeters)
    bmr = basal_metabolic_rate(gender, kilograms, centimeters, years)
    # Print the results for the user to see.
    print(f"Age (years): {years}\nWeight (kg): {kilograms:.2f}")
    print(f"Height (cm): {centimeters}\nBody Mass Index: {bmi:.2f}")
    print(f"Basal Metabolic Rate (kcal/day): {bmr:.2f}")
    pass

def compute_age(birth_str):
    """Compute and return a person's age in years.
    Parameter birth_str: a person's birthdate stored
        as a string in this format: YYYY-MM-DD
    Return: a person's age in years.
    """
    # Convert a person's birthdate from a string
    # to a date object.
    birthdate = datetime.strptime(birth_str, "%Y-%m-%d")
    today = datetime.now()

    # Compute the difference between today and the
    # person's birthdate in years.
    years = today.year - birthdate.year

    # If necessary, subtract one from the difference.
    if birthdate.month > today.month or \
        (birthdate.month == today.month and \
            birthdate.day > today.day):
        years -= 1

    return years


def kg_from_lb(pounds):
    """Convert a mass in pounds to kilograms.
    Parameter pounds: a mass in U.S. pounds.
    Return: the mass in kilograms.
    """
    kilograms = pounds * .45359237
    return kilograms


def cm_from_in(inches):
    """Convert a length in inches to centimeters.
    Parameter inches: a length in inches.
    Return: the length in centimeters.
    """
    centimeters = inches * 2.54
    return centimeters


def body_mass_index(kilograms, centimeters):
    """Compute and return a person's body mass index.
    Parameters
        weight: a person's weight in kilograms.
        height: a person's height in centimeters.
    Return: a person's body mass index.
    """
    bmi = 10000 * kilograms / centimeters**2
    return bmi


def basal_metabolic_rate(gender, kilograms, centimeters, years):
    """Compute and return a person's basal metabolic rate.
    Parameters
        weight: a person's weight in kilograms.
        height: a person's height in centimeters.
        age: a person's age in years.
    Return: a person's basal metabolic rate in kcals per day.
    """
    if gender.lower() == "female":
        bmr = 447.593 + 9.247*kilograms + 3.098*centimeters - 4.330*years

    elif gender == "Male":
        bmr = 88.362 + 13.397*kilograms + 4.799*centimeters - 5.677*years

    else:
        bmr = "You didn't enter Male or Female as your gender so bmr cannot be calculated. "

    return bmr


# Call the main function so that
# this program will start executing.
main()
